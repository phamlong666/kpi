...
if "spreadsheet_id" not in st.session_state: st.session_state["spreadsheet_id"] = GOOGLE_SHEET_ID_DEFAULT
if "kpi_sheet_name"  not in st.session_state: st.session_state["kpi_sheet_name"]  = KPI_SHEET_DEFAULT
if "drive_root_id"   not in st.session_state: st.session_state["drive_root_id"]   = APP_KPI_DRIVE_ROOT_ID_DEFAULT
if "_report_folder_id" not in st.session_state: st.session_state["_report_folder_id"] = ""
...
def open_spreadsheet(sid_or_url: str):
    ...
    if gclient is None: raise RuntimeError("Chưa có Google client (secrets).")
...
ALIAS = {
    ...
    "Tháng":["Tháng","Month"], "Năm":["Năm","Year"],
    "Điểm KPI":["Điểm KPI","Score"], "Ghi chú":["Ghi chú","Notes"],
    "Tên đơn vị":["Tên đơn vị","Đơn vị"],
    "Phương pháp đo kết quả":["Phương pháp đo kết quả","Cách tính","Công thức"],
    "Ngưỡng dưới":["Ngưỡng dưới","Min"], "Ngưỡng trên":["Ngưỡng trên","Max"],
}
def normalize_columns(df: pd.DataFrame)->pd.DataFrame:
    ...
    for std, cands in ALIAS.items():
        ...
        for c in cands:
            key = c.strip().lower()
            if key in cols_lower: rename[cols_lower[key]] = std; break
    if rename: df = df.rename(columns=rename)
    if "Thực hiện (tháng)" in df.columns and "Thực hiện" not in df.columns: df = df.rename(columns={"Thực hiện (tháng)":"Thực hiện"})
...
def compute_score_with_method(row):
    ...
    if any(k in method for k in ["tăng",">=","cao hơn tốt","increase","higher"]):
        ratio = max(min(actual/plan,2.0),0.0); return round(ratio*10*w,2)
    if any(k in method for k in ["giảm","<=","thấp hơn tốt","decrease","lower"]):
        ratio = 1.0 if actual<=plan else max(min(plan/actual,2.0),0.0); return round(ratio*10*w,2)
    if any(k in method for k in ["đạt","dat","bool","pass/fail"]):
        ok = actual>=plan; return round((10.0 if ok else 0.0)*w,2)
    if any(k in method for k in ["khoảng","range","trong khoảng"]):
        ...
        if lo is None or hi is None: return compute_score_generic(plan, actual, weight)
        ok = lo <= actual <= hi; return round((10.0 if ok else 0.0)*w,2)
...
def check_credentials(use_name:str, password:str)->bool:
    ...
    col_use = next((c for c in df.columns if c.strip().lower() in
                   ["tài khoản (use\\username)","tài khoản","username","use (mã đăng nhập)","use"]), None)
    col_pw  = next((c for c in df.columns if c.strip().lower() in
                   ["mật khẩu mặc định","password mặc định","password","mật khẩu"]), None)
...
def get_drive_service():
    ...
    gclient, creds = st.session_state.get("_gs_pair",(None,None))
    if creds is None:
        gclient, creds = get_gs_clients(); st.session_state["_gs_pair"] = (gclient,creds)
...
def ensure_parent_ok(service, parent_id:str):
    try: service.files().get(fileId=parent_id, fields="id,name").execute()
        ...
        raise RuntimeError(f"Không truy cập được thư mục gốc (ID: {parent_id}). "
                           "Kiểm tra ID/URL & quyền Editor cho service account.") from e
    ...
    if "_user" not in st.session_state:
        ...
        if login_submit:
            if check_credentials(use_input, pwd_input):
                st.session_state["_user"] = use_input.strip(); toast("Đăng nhập thành công.","✅"); st.rerun()
            else: st.error("USE hoặc mật khẩu không đúng.")
    else:
        ...
        if st.button("🔧 Chuẩn bị thư mục báo cáo", use_container_width=True):
            ...
            except Exception as e: st.error(f"Lỗi chuẩn bị thư mục: {e}")
...
def get_sheet_and_name():
    ...
    sh = open_spreadsheet(sid_cfg); return sh, sheet_name

def write_kpi_to_sheet(sh, sheet_name:str, df:pd.DataFrame)->bool:
    df = normalize_columns(df.copy())
    if "Điểm KPI" not in df.columns: df["Điểm KPI"] = df.apply(compute_score_with_method, axis=1)
    ...
    try:
        ...
        except Exception: ws = sh.add_worksheet(title=sheet_name, rows=len(data)+10, cols=max(12,len(cols)))
...
with st.container(border=True):
    ...
    # H2 (Kế hoạch/Thực hiện: text VN + on_change)
    c1 = st.columns(3)
...
# ===== NÚT =====
if apply_clicked:
    apply_form_to_cache(update_selected=True); toast("Đã áp dụng dữ liệu biểu mẫu vào CSV tạm.","✅"); st.rerun()

def generate_pdf_from_df(df: pd.DataFrame, title="BÁO CÁO KPI")->bytes:
    try:
        from reportlab.lib.pagesizes import A4, landscape
        from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer
        from reportlab.lib import colors; from reportlab.lib.units import cm
        from reportlab.lib.styles import getSampleStyleSheet
        buf = io.BytesIO()
        doc = SimpleDocTemplate(buf, pagesize=landscape(A4), rightMargin=20,leftMargin=20,topMargin=20,bottomMargin=20)
        styles = getSampleStyleSheet(); story=[Paragraph(title, styles["Title"]), Spacer(1,0.3*cm)]
        cols = list(df.columns); data = [cols] + df.fillna("").astype(str).values.tolist()
...
if save_csv_clicked:
    ...
    except Exception as e: st.error(f"Lỗi khi ghi Sheets: {e}")

if refresh_clicked:
    if "confirm_refresh" not in st.session_state: st.session_state["confirm_refresh"]=True
    else: st.session_state["confirm_refresh"]=not st.session_state["confirm_refresh"]
...
if save_drive_clicked:
    try:
        ...
        fname_xlsx, fname_pdf = f"KPI_{ts}.xlsx", f"KPI_{ts}.pdf"
        buf_xlsx = io.BytesIO()
        with pd.ExcelWriter(buf_xlsx, engine="xlsxwriter") as writer:
            st.session_state["_csv_cache"].to_excel(writer, index=False, sheet_name="KPI")
    ...
    except Exception as e: st.error(f"Lỗi lưu Google Drive: {e}")
